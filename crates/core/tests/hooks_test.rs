...
use claude_agent_core::hooks::{HookInput, HookOutput, HookRegistry};
...
fn registry_register_single() {
    ...
    let cb: claude_agent_core::hooks::HookCallback =
...
fn registry_register_different_events() {
    let mut registry = HookRegistry::new();
    let cb1: claude_agent_core::hooks::HookCallback =
        Arc::new(|_input, _id, _ctx| Box::pin(async { Ok(HookOutput::default()) }));
    let cb2: claude_agent_core::hooks::HookCallback =
        Arc::new(|_input, _id, _ctx| Box::pin(async { Ok(HookOutput::default()) }));
...
async fn execute_runs_callback() {
    let mut registry = HookRegistry::new();
    let cb: claude_agent_core::hooks::HookCallback = Arc::new(|input, _id, _ctx| {
        Box::pin(async move {
            Ok(HookOutput {
                continue_execution: true,
...
async fn execute_matcher_filters() {
    let mut registry = HookRegistry::new();
    let cb: claude_agent_core::hooks::HookCallback = Arc::new(|_input, _id, _ctx| {
        Box::pin(async {
