...
fn benchmark_rate_limiter(c: &mut Criterion) {
    use claude_agent::mcp::rate_limiter::{RateLimitConfig, RateLimiter};
...
fn benchmark_transport_factory(c: &mut Criterion) {
    use claude_agent::mcp::create_mcp_server;
    use claude_agent::types::config::{McpServerConfig, McpTransportType};
